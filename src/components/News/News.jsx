...
const News = ({ item }) => {
    ...
    const fourStarRating = <div className='flex items-center gap-2'>
        <MdOutlineStar color='#FFD700' size={20} />
        <MdOutlineStar color='#FFD700' size={20} />
        <MdOutlineStar color='#FFD700' size={20} />
        <MdOutlineStar color='#FFD700' size={20} />
        <MdOutlineStarBorder color='#FFD700' size={20} />
    </div>

    const threeStarRating = <div className='flex items-center gap-2'>
        <MdOutlineStar color='#FFD700' size={20} />
        <MdOutlineStar color='#FFD700' size={20} />
        <MdOutlineStar color='#FFD700' size={20} />
        <MdOutlineStarBorder color='#FFD700' size={20} />
    ...
    const publishedDate = new Date(item.author.published_date).toLocaleDateString();

    return (
        <NavLink to={`/categories/details/${item.category_id}}`}>
            <div className="flex flex-col w-full p-6 space-y-6 overflow-hidden rounded-lg border border-base-300 shadow-sm dark:bg-gray-50 dark:text-gray-800">
                <div className="flex justify-between items-center">
                    <div className='flex space-x-4'>
                        ...
                        <div className="flex flex-col space-y-1">
                            ...
                            <span className="text-xs dark:text-gray-600">{publishedDate}</span>
                        </div>
                    </div>
                ...
                <div>
                    <img src={item.thumbnail_url} alt="" className="object-cover w-full mb-4 h-60 sm:h-96 dark:bg-gray-500" />
                    <h2 className="mb-1 text-xl font-semibold">{item.title}</h2>
                    <div>
                        <p className={`${isReadMore ? '' : 'truncate'}`}>
                            {item.details}
                        </p>
                        <button className='text-secondary font-bold' onClick={() => setIsReadMore(!isReadMore)}>
                            {
                                isReadMore ?
                                    'Read less' :
                                    'Read more'
                ...
                <div className='flex items-center gap-4'>
                    <div>
                        {
                            item.rating.number === 5 && fiveStarRating
                        }
                        {
                            item.rating.number === 4 && fourStarRating
                        }
                        {
                            item.rating.number === 3 && threeStarRating
                        }
                    </div>
                    <p className='text-sm font-semibold'>{item.rating.number}.00</p>
